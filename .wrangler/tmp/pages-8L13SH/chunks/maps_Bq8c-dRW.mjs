...
class DatabaseService {
  ...
  getDB() {
    try {
      console.log("Checking for DB instance...");
      if (typeof Astro !== "undefined") {
        console.log("Astro is defined");
        if (Astro.locals) {
          console.log("Astro.locals is defined");
          if (Astro.locals?.runtime?.env?.DB) {
            console.log("Found DB in Astro.locals.runtime.env.DB");
            return Astro.locals.runtime.env.DB;
          } else if (Astro.locals?.DB) {
            console.log("Found DB in Astro.locals.DB");
            return Astro.locals.DB;
          } else {
            console.log("No DB found in Astro.locals");
          }
        } else {
          console.log("Astro.locals is not defined");
        }
      } else {
        console.log("Astro is not defined");
      }
      if (typeof env !== "undefined") {
        console.log("env is defined");
        if (env.DB) {
          console.log("Found DB in env.DB");
          return env.DB;
        } else {
          console.log("No DB found in env");
